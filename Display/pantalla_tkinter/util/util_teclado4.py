...
class TecladoNumerico4(tk.Toplevel):
    def __init__(self, master, entry, es_indice=False):
        ...
        self.entry_display = tk.Entry(self.frame_principal, font=("Helvetica", 14),textvariable=self.entry, state="normal", bg="white", fg="black", width=20)
    ...
    def agregar_caracter(self, caracter):
        ...
        current_text = self.entry.get().replace('%', '')  # Eliminar % si existe
        
        if caracter == 'Borrar':
            # Actualizar ambos entries de una sola vez
            ...
            if caracter in '0123456789.':
                if len(current_text) < 15:
                    # Actualizar el contenido sin forzar updates
                    new_text = current_text + caracter
                    self.entry.config(state='normal')
                    self.entry.delete(0, tk.END)
                    self.entry.insert(0, new_text)
                    self.entry_display.delete(0, tk.END)
                    self.entry_display.insert(0, new_text)

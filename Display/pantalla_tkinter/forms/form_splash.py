...
class SplashScreen(tk.Tk):
    def __init__(self):
        ...
        # Opcionalmente forzar fullscreen total si el OS lo permite
        # self.attributes("-fullscreen", True)

        # Atajo para cerrar la aplicación
        self.bind_all("<Control-q>", lambda e: self.desbloquear_y_salir())
    ...
    def desbloquear_y_salir(self):
        """Sale del mainloop si está bloqueado y destruye la ventana."""
        try:
            self.quit() # Detiene el mainloop
            self.destroy() # Cierra la ventana
            import sys
            sys.exit(0) # Finaliza el proceso python
        except:
            import sys
            sys.exit(0)

...
def existe_curva(nombre: str) -> bool:
    return db_local.existeCurva(nombre)


def guardar_curva(nombre: str, segmentos: list):
    return db_local.guardarCurva(nombre, segmentos)
...
class CurvaAlimentacionWindow(tk.Toplevel):
    def __init__(self, master, logo):
        ...
        self.entry_nombre.bind(
            '<Button-1>',
            lambda e: self.mostrar_teclado_alfanumerico(self.entry_nombre)
        )

        # ===== Tabla de segmentos =====
        self.frame_segmentos = tk.Frame(self, bg="#EF9480")
        self.frame_segmentos.pack(
            side=tk.TOP,
            fill="both",
            padx=20,
            pady=5,
        ...
        # Lista con las filas (frame, entry_dia, entry_indice)
        self.segmentos = []
        self.iniciar_tabla_segmentos()

        # ===== Botones + y - =====
        self.frame_botonera_segmentos = tk.Frame(self, bg="#EF9480")
        self.frame_botonera_segmentos.pack(side=tk.TOP, fill="x", pady=(10, 0))

        self.boton_agregar_segmento = tk.Button(
            self.frame_botonera_segmentos,
            text="+",
            font=("Helvetica", 16),
            command=self.agregar_segmento,
            bg="red",
            fg="white",
            bd=7
        )
        self.boton_agregar_segmento.pack(side=tk.LEFT, padx=10, pady=5)

        self.boton_eliminar_segmento = tk.Button(
            self.frame_botonera_segmentos,
            text="-",
            font=("Helvetica", 16),
            command=self.eliminar_segmento,
            bg="red",
            fg="white",
            bd=7
        )
        ...
        self.boton_guardar.pack(side=tk.RIGHT, padx=10, pady=10)
    ...
    def iniciar_tabla_segmentos(self):
        """Una sola fila de segmento al principio."""
    ...
    def crear_segmento(self, index):
        ...
        if index == 1:
            ...
            entry_dia.insert(0, str(self.calcular_dia()))
        entry_dia.grid(row=0, column=1, padx=72)
        entry_dia.bind(
            '<Button-1>',
            lambda e: util_teclado2.TecladoNumerico(self, entry_dia, es_indice=False)
        )
        ...
        entry_indice.insert(0, "")
        entry_indice.grid(row=0, column=3, padx=72)

        validate_indice = self.register(self.validar_indice)
        entry_indice.config(
            validate="key",
            validatecommand=(validate_indice, "%P")
        )
        entry_indice.bind(
            '<Button-1>',
            lambda e: util_teclado2.TecladoNumerico(self, entry_indice, es_indice=True)
        )
    ...
    def agregar_segmento(self):
        ...
        if len(self.segmentos) < 17:
            self.crear_segmento(len(self.segmentos) + 1)
        else:
            mostrar_mensaje(
                self,
                "Límite alcanzado",
                "Ya no se pueden agregar más segmentos.",
                "warning"
            )
    ...
    def eliminar_segmento(self):
        if len(self.segmentos) > 1:
            ...
            mostrar_mensaje(
                self,
                "Límite alcanzado",
                "Debe haber al menos un segmento.",
                "warning"
            )
    ...
    def guardar_configuracion(self):
        ...
        if not config_name:
            mostrar_mensaje(
                self,
                "Error",
                "El nombre de la curva no puede estar vacío",
                "error"
            )
            return

        # No permitir nombres reservados
        nombre_bloqueado = config_name.strip().lower() in [
            "ascendente",
            "constante",
            "descendente",
            "forma v"
        ...
        if nombre_bloqueado:
            mostrar_mensaje(
                self,
                "Error",
                "No se puede usar ni modificar las curvas Ascendente/Constante/Descendente/Forma V.",
                "error"
            )
            return
        ...
        if existe_curva(config_name):
            mostrar_mensaje(
                self,
                "Error",
                "Ya existe una curva con este nombre",
                "error"
            )
            return
        ...
        for _, entry_dia, entry_indice in self.segmentos:
            ...
            if not dia.isdigit():
                mostrar_mensaje(
                    self,
                    "Error",
                    f"Día inválido: {dia} - debe ser un número",
                    "error"
                )
                return
            ...
            if dia_num <= prev_day:
                mostrar_mensaje(
                    self,
                    "Error",
                    f"Día {dia_num} no es incremental",
                    "error"
                )
                return
            ...
            if dia_num > 114:
                mostrar_mensaje(
                    self,
                    "Error",
                    f"Día {dia_num} excede el límite de 114",
                    "error"
                )
                return
            ...
            if not self.validar_indice(indice):
                mostrar_mensaje(
                    self,
                    "Error",
                    f"Índice inválido: {indice} - debe ser entre 1 y 200",
                    "error"
                )
                return
            ...
            if int(indice.replace('%', '')) < 1:
                mostrar_mensaje(
                    self,
                    "Error",
                    "El índice debe ser mayor que 0",
                    "error"
                )
                return
        ...
        if has_empty_index:
            mostrar_mensaje(
                self,
                "Error",
                "Todos los índices deben tener un valor",
                "error"
            )
            return

        # Limitar a 2 curvas personalizadas en total
        curvas_existentes = db_local.cargarCurvas()
        nombres_defecto = ["ascendente", "constante", "descendente", "forma v"]
        curvas_nuevas = [
            c for c in curvas_existentes
            if c["nombre"]
            and c["nombre"].strip().lower() not in nombres_defecto
        ]
        if len(curvas_nuevas) >= 2:
            mostrar_mensaje(
                self,
                "Error",
                "Solo se pueden agregar hasta 2 curvas personalizadas.",
                "error"
            )
            return
        ...
        if success:
            ...
            mostrar_mensaje(
                self,
                "Éxito",
                f"Curva '{config_name}' guardada correctamente\n\nPresione OK para continuar",
                "info",

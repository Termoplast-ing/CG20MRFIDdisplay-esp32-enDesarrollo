    ...
    def guardar_y_volver(self):
        try:
            ...
            for caravana, inseminacion in self.datos_seleccionados:
                ...
                #Agregamos esto nuevo y comentamos el resto:
                ts_ins = int(inseminacion)
                
                #if "/" in inseminacion:
                #    dia, mes, año = inseminacion.split("/")
                #    fecha_para_timestamp = f"{año}-{mes}-{dia}"
                #else:
                #    fecha_para_timestamp = inseminacion

                #ts_ins = int(datetime.strptime(fecha_para_timestamp, "%Y-%m-%d").timestamp())
                # Comentamos Hasta Aca
            ...
            #import util.util_mensaje
            #util.util_mensaje.mostrar_mensaje(self, "Guardado", "La dieta se guardó en SQLite y se envió al equipo.", "info")
